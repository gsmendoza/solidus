...
task :clean do
  ...
  %w(api backend core sample).each do |gem_name|
...
namespace :gem do
  ...
  def for_each_gem
    %w(core api backend sample).each do |gem_name|
      print_title(gem_name)
